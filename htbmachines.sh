...
function getSkill(){
    ...
    check_skill="$(cat bundle.js| grep "skills: " -B 6 | grep "$kill" -i -B 6 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column)"

    if [ "$check_skill" ];then
        echo -e "\n${yellowColour}[+]${endColour} A continuacion se representan las maquinas donde se ve la tecnica${endColour} ${blueColour}$skill${endColour}"
        cat bundle.js| grep "skills: " -B 6 | grep "$skill" -i -B 6 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column
    else
        echo -e "\n${redColour}[!] No se ha encontrado con la skill indicada${endColour}\n"
    fi
}
